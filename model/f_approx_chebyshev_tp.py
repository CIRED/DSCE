...
def Bell_max_uncertain_boundary(y):
    ...
    #expected value
        ...
        un_min_adj=adjust_min(un_min,un_max,m)
        un_max_adj=adjust_max(un_min,un_max,m)
        v1=maximand_pretipping(array([t_step-1]),x_step,u_step,eps)+bet*approx_Chebyshev(coefn,weight_cheb,un_adj,un_min_adj,un_max_adj)-out_penality*out_interval(x_step,zeros(shape(x_step)),ones(shape(x_step)))-out_penality*out_interval(un,un_min,un_max)
        v=float(-v1)#minus because we will minimize this function (so as to maximize the Bellman function)
    return v;
...
def Bell_max_uncertain_2(x):
    ...
    if t_step==T:
        ...
        v1=maximand_pretipping(array([t_step-1]),x_step,u_step,eps)+bet*((1-p)*terminal_value_pretipping(t_step+1,un)+p*terminal_value_posttipping(t_step+1,un))-out_penality*out_interval(x_step,zeros(shape(x_step)),ones(shape(x_step)))-out_penality*out_interval(un,un_min,un_max2)
        ...
        if isnan(p):
            print('Error: p is a nan')
            print(u_step[0][1],un[0][1])
        v1=maximand_pretipping(array([t_step-1]),x_step,u_step,eps)+bet*((1-p)*approx_Chebyshev(coefn,weight_cheb,un_adj,un_min_adj,un_max_adj)+p*approx_Chebyshev(coefn2,weight_cheb,un_adj2,un_min_adj2,un_max_adj2))-out_penality*out_interval(x_step,zeros(shape(x_step)),ones(shape(x_step)))-out_penality*out_interval(un,un_min,un_max2)
...
def Bell_max_pretipping_3interpolations(x):
    ...
    #expected value
        ...
        if un[0][1]<=u_threshold_min:
            ...
            v1=maximand_pretipping(array([t_step-1]),x_step,u_step,eps)+bet*((1-p)*approx_Chebyshev(coefn2,weight_cheb,un_adj2,un_min_adj2,un_max_adj2)+p*approx_Chebyshev(coefn3,weight_cheb,un_adj3,un_min_adj3,un_max_adj3))-out_penality*out_interval(x_step,zeros(shape(x_step)),ones(shape(x_step)))-out_penality*out_interval(un,un_min2,un_max2)
