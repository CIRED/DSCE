...
def maximand_pretipping(t,x,u,eps):
    ...
    conso=damage_factor_pretipping(temperature(s),eps)*(1-abatement_cost(a,theta1[t,:]))*production(A[t,:],k,L[t,:])/L[t,:]*c
    v=1/L[[0],:]*L[t,:]*utility(conso,s)
    return v;
    
def maximand_posttipping(t,x,u,eps):
    x=maximum(x,zeros(shape(x)))
    x=minimum(x,ones(shape(x)))
    c=x[:,[0]]#c is relative consumption
    a=x[:,[1]]#a is relative abatement
    k=(A[t,:]**(1/(1-alpha)))*L[t,:]*u[:,[0]]#u[:,[0]] is capital per efficient capita
    s=u[:,[1]]#total emissions
    conso=damage_factor_posttipping(temperature(s),eps)*(1-abatement_cost(a,theta1[t,:]))*production(A[t,:],k,L[t,:])/L[t,:]*c
    v=1/L[[0],:]*L[t,:]*utility(conso,s)
    return v;
...
#t is time 1D-array of size 1 or lx
#v is maximand value of size lx*1
def partialc_maximand_pretipping(t,x,u,eps):
    x=maximum(x,zeros(shape(x)))
    x=minimum(x,ones(shape(x)))
    c=x[:,[0]]#c is relative consumption
    a=x[:,[1]]#a is relative abatement
...
def partialc_maximand_posttipping(t,x,u,eps):
    x=maximum(x,zeros(shape(x)))
    x=minimum(x,ones(shape(x)))
    c=x[:,[0]]#c is relative consumption
    a=x[:,[1]]#a is relative abatement
...
def law_motion_pretipping(t,x,u,eps):
    x=maximum(x,zeros(shape(x)))
    x=minimum(x,ones(shape(x)))
    c=x[:,[0]]#c is relative consumption
    a=x[:,[1]]#a is relative abatement
    ...
    s=u[:,[1]]#total emissions
    y=production(A[[t-1],:],k,L[[t-1],:])*damage_factor_pretipping(temperature(s),eps)
    kn=((1-delta))*k*damage_factor_pretipping(temperature(s),eps)**(damage_capital/alpha)+y*(1-abatement_cost(a,theta1[[t-1],:]))*(1-c)
    sn=s+emissions(a,y,sigm[[t-1],:])
    un=concatenate((kn/((A[[t],:]**(1/(1-alpha)))*L[[t],:]),sn),axis=1)
    return un;
    
    
def law_motion_posttipping(t,x,u,eps):
    x=maximum(x,zeros(shape(x)))
    x=minimum(x,ones(shape(x)))
    c=x[:,[0]]#c is relative consumption
    a=x[:,[1]]#a is relative abatement
    k=(A[[t-1],:]**(1/(1-alpha)))*L[[t-1],:]*u[:,[0]]#u[:,[0]] is capital per efficient capita
    s=u[:,[1]]#total emissions
    y=production(A[[t-1],:],k,L[[t-1],:])*damage_factor_posttipping(temperature(s),eps)
    kn=((1-delta))*k*damage_factor_posttipping(temperature(s),eps)**(damage_capital/alpha)+y*(1-abatement_cost(a,theta1[[t-1],:]))*(1-c)
    sn=s+emissions(a,y,sigm[[t-1],:])
    un=concatenate((kn/((A[[t],:]**(1/(1-alpha)))*L[[t],:]),sn),axis=1)
    return un;
...
def terminal_value_posttipping(t,u):
    if not(t==T+1):
    ...
    a=1
    #a=0
    conso=(damage_factor_posttipping(temperature(s),eps)*(1-abatement_cost(a,theta1[[t-1],:]))*production(A[[t-1],:],k,L[[t-1],:])+((1-delta))*k*damage_factor_posttipping(temperature(s),eps)**(damage_capital/alpha)-kn)/L[[t-1],:]
...
#terminal scrap-value
#sum of the utilities from T+1 to infty
def terminal_value_pretipping(t,u):
    if not(t==T+1):
        print('Are you sure terminal_value function is correctly called?')
    kpec=u[:,[0]]
    s=u[:,[1]]
    ...
    a=1
    #a=0
    conso=(damage_factor_pretipping(temperature(s),eps)*(1-abatement_cost(a,theta1[[t-1],:]))*production(A[[t-1],:],k,L[[t-1],:])+((1-delta))*k*damage_factor_pretipping(temperature(s),eps)**(damage_capital/alpha)-kn)/L[[t-1],:]
    #adjustement in terminal constraint adapted from Barr Manne (GA is growth rate per year, not per period) 
    v=1/L[[0],:]*L[[t-1],:]*1/(1-bet*((1+GA[[t-1],:])**(deltaT*(1-theta)/(1-alpha))))*utility(conso,s)
    return v;
