...
coef_V_posttipping=zeros((T,nb_terms_cheb))
...
print('interpolation post tipping')

for t_step in list(range(T,0,-1)):
    #print('interpolation post tipping in step '+str(t_step))
    u_adjust=(z_adjust+1)/(-2*z_adjust)*(u_max_posttipping[[t_step-1],:]-u_min_posttipping[[t_step-1],:])
    ...
    for i in list(range(m**dim)):
        ...
        u[[t_step-1],:]=(z[[i],:]+1)/2 *(u_max_posttipping[[t_step-1],:]-u_min_posttipping[[t_step-1],:]+2*u_adjust)+u_min_posttipping[[t_step-1],:]-u_adjust
        ...
        approx_value=-Bell_max_posttipping(xopt)
        guess_store[t_step-1,:,i]=xopt.copy()
        error_guess[t_step-1,:,i]=guess-xopt
        guess=xopt.copy()
        for j in list(range(nb_terms_cheb)):
            coef_V_posttipping[t_step-1,j]=coef_V_posttipping[t_step-1,j]+2**(d_cheb[[j],:])/(m**dim)*approx_value*Chebyshev(weight_cheb[[j],:],z[[i],:])
